...
def buildModel(lossParam, optimizerParam, activationParam, nodes):
    model = keras.Sequential([
        layers.Dense(nodes, activation=activationParam, input_shape=[len(training_data.keys())]),
        layers.Dense(nodes, activation=activationParam),
        layers.Dense(1)
    ])
...
# model = buildModel('mae', tf.keras.optimizers.RMSprop(0.001), tf.nn.relu, 32)
...
# Use EarlyStopping callback to test a training condition to stop training when the validation score stops improving.
    ...
    model = buildModel('mse', tf.keras.optimizers.SGD(lr=0.001), tf.nn.sigmoid, 2**nodes)
    early_stop = keras.callbacks.EarlyStopping(monitor='val_loss', patience=10)

    history = model.fit(normTrainingData, train_labels, batch_size=32, epochs=1500,
