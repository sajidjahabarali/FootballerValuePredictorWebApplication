...
def buildModel(current_layer1, current_layer2):
    model = keras.Sequential([
        ...
        keras.layers.Dense(current_layer1, activation=tf.nn.sigmoid),
        keras.layers.Dense(current_layer2, activation=tf.nn.sigmoid),
...
layer1 = [32, 64, 128, 256]
layer2 = [32, 64, 128, 256]

bestL1 = -1;
bestL2 = -1;
bestAccuracy = -1;

for current_layer1 in layer1:
    for current_layer2 in layer2:
    # build the model and store it in a variable.
        model = buildModel(current_layer1, current_layer2)

        # print('==================================================')
        # print('TRAINING')
        # train the model
        model.fit(training_data, training_labels, epochs=320, batch_size=16, verbose=0)

        # print('==================================================')
        # print('ACCURACY TESTING')
        # check accuracy
        test_loss, test_accuracy = model.evaluate(testing_data, testing_labels, verbose=0)
        # print('Test accuracy:', test_accuracy)
        if(test_accuracy>bestAccuracy):
            bestAccuracy = test_accuracy
            bestL1 = current_layer1
            bestL2 = current_layer2
