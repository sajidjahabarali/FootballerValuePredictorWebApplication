...
model = buildModel('mse', tf.keras.optimizers.SGD(lr=0.001), tf.nn.sigmoid)
...
PLAYER_DATA_PATH = 'C:\\Users\\sajid\\OneDrive\\Documents\\University\\Final Year Project\\Code\\FootballerNetWorthPredictor\\web application\\flask\\data\\data.csv'

input_player_data = pd.read_csv(PLAYER_DATA_PATH, names=column_names, na_values="?", comment='\t', sep=",", skipinitialspace=True,
                          encoding='latin-1')

input_player_data = position_one_hot(input_player_data.dropna())
input_player_data.pop('Value(€M)')
